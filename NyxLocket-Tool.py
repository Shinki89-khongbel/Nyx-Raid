...
# Discord: Nyx
# Version: 1.0.6 (Fixed)
# Github: https://github.com/Shinki89-khongbel
# Description: NyxLocket Tool Open Source
# ==================================
import sys
import subprocess
def _install_():
...
class Nyx:
    def __init__(self, device_token: str="", target_friend_uid: str="", num_threads: int=1, note_target: str=""):
        ...
        self.NAME_TOOL="NyxLocket Tool Pro"
        ...
        if os.name == "nt":
            os.system(
                f"title 💰 {self.NAME_TOOL} {self.VERSION_TOOL} by Nyx 💰"
         )
    ...
    def excute(self, url, headers=None, payload=None, thread_id=None, step=None, proxies_dict=None):
        ...
        try:
            ...
            return None
    def setup(self):
        self._nyxlocket_panel_()
    def _input_(self, prompt_text="", section="config"):
        print(
            f"{xColor.CYAN}┌──({xColor.NEON_GREEN}root@nyx{xColor.CYAN})-[{xColor.PURPLE}{section}{xColor.CYAN}]")
        ...
        sys.stdout.flush()
        return input()
    def _nyxlocket_panel_(self):
        _clear_()
        print(
            f"\n{xColor.CYAN}╔═══════════════════════════════════════════════════════╗")
        print(f"{xColor.CYAN}║ {xColor.YELLOW}            LOCKET LAUNCHER ADVANCED PANEL            {xColor.CYAN}║")
        print(
            f"{xColor.CYAN}║ {xColor.RED}                 [discord: @{self.discord}]                {xColor.CYAN}║")
        print(
            f"{xColor.CYAN}╚═══════════════════════════════════════════════════════╝\n")
        ...
        nyxlocket_confirm=self._input_(
        ...
        if nyxlocket_confirm.strip().lower() in ('y', 'yes', '1'):
            self.nyxlocket_confirm=True
        else:
...
def _banner_():
    ...
    banner=[
        ...
        f"{xColor.RED}╚══════{xColor.GREEN}╝░░░░░░{xColor.RED}╚══════{xColor.GREEN}╝░{xColor.RED}╚════╝{xColor.GREEN}░░{xColor.RED}╚════╝{xColor.GREEN}░{xColor.RED}╚═{xColor.GREEN}╝░░{xColor.RED}╚═{xColor.GREEN}╝{xColor.RED}╚══════{xColor.GREEN}╝░░░{xColor.RED}╚═{xColor.GREEN}╝░░░",
        f"{xColor.WHITE}[ {xColor.YELLOW}Author: @{config.author} {xColor.RED}|{xColor.WHITE} {xColor.GREEN}nyxlocket Tool {config.VERSION_TOOL}{xColor.WHITE} ]"
    ]
...
def main():
    ...
    typing_print(
        f"-----------------[NyxLocket Tool Pro {config.VERSION_TOOL}]-----------------", delay=0.01)
    config._print(
